...
ScrollReveal().reveal('.hero-content', { delay: 300, origin: 'bottom', distance: '50px' });
ScrollReveal().reveal('.about', { delay: 300, origin: 'left', distance: '50px' });
ScrollReveal().reveal('.service-card', { interval: 200, origin: 'bottom', distance: '50px' });
...
// Particle Background with Mouse Interactivity
...
function drawParticles(){
  ...
  for(let i=0;i<particles.length;i++){
    let p = particles[i];
    ctx.beginPath(); ctx.arc(p.x,p.y,p.radius,0,Math.PI*2); ctx.fillStyle="rgba(0,255,255,0.7)"; ctx.fill();
  }

  for(let i=0;i<particles.length;i++){
    for(let j=i+1;j<particles.length;j++){
      let dx = particles[i].x-particles[j].x; let dy = particles[i].y-particles[j].y; let dist = Math.sqrt(dx*dx+dy*dy);
      if(dist<120){ ctx.beginPath(); ctx.strokeStyle="rgba(0,255,255,"+(1-dist/120)+")"; ctx.lineWidth=1; ctx.moveTo(particles[i].x,particles[i].y); ctx.lineTo(particles[j].x,particles[j].y); ctx.stroke(); }
    }
  }
